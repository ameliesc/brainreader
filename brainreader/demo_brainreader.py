...
def demo_brainreader():
    ...
    # Need to get dataset.  See from scipy.io import loadmat

    raw_content_image = get_image('starry_night', size=(128, None))
    input_im = normalize(im2feat(raw_content_image))
    net = get_vgg_net(up_to_layer='conv2_1')
    func = net.compile()
    feat = func(input_im)  # shape (n_samples, n_maps, size_y, size_x)

    plt.subplot(2,1,1)
    plt.imshow(raw_content_image)
    plt.title('Image')
    plt.subplot(2,1,2)
    plt.imshow(put_data_in_grid(feat.reshape(16, 8, *feat.shape[2:])), cmap='gray', interpolation = 'nearest')
    plt.title('Features')
    plt.show()
