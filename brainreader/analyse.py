...
def filtervoxels(layer_name, n = 1):
    ...
    for i in range (0,120): #   conv5_new[i] = (conv5[i][np.where(np.less(conv5[i] < conv5_i[i]))], np.where(np.less(conv5[i] < conv5_i[i])))
...
def avgcost(roi):  
    ...
    for layer in ['fc6', 'fc7', 'fc8']:
        ...
        index = filtercost(dic)
        cost = dd.io.load('/data/regression_cost_roi%s_%s.h5' % (roi,layer))
        layers[layer] = sum(cost[0][index])/len(cost[0][index])
    return layers
